<template>
	<v-card>
      ...
      <div 
        ...
        <div class="item height" @click="fromClipBoard(copiedToClipboard)">&uArr;</div>
		...
		methods:{
			...
			procent(){
				if (this.dislay1 !== '' && this.display2 !== '' && this.signDisplay !== ''){
					...
					let sum = this.display1 * this.display2 / 100
						this.display1 = (eval(this.display1 + this.signDisplay + sum).toFixed(2)).toString();
					}else if(this.signDisplay === '*'){
					// let sum = this.display1 * this.display2 / 100
					this.display1 = ((eval(this.display1 * this.display2) / 100).toFixed(2)).toString();
					}else{
					this.display1 = ((eval(this.display1 * 100 / this.display2)).toFixed(2)).toString();
					}
					this.display2 = ''
					this.signDisplay = ''
				}
			...
			incertDot(){
				...
				// if (!this.dot){
				// 	this.display2 = this.display2 + num.toString();
				// 	this.equaled = false
				// 	this.operator = false
				// 	this.dot = true
				// 	this.num = false
			...
			equal(){
				...
				if (this.display2 !== '' && this.display1 !== '' ){
							this.display1 = (eval(this.display1 + this.signDisplay + this.display2).toFixed(2)).toString();
							this.display2 = ''
							this.signDisplay = ''
						}
			}
